	...
	public static void setBackground(int w, int h){
		...
		for(double x=0;x<w;x++){
			for(double y=0;y<h;y++){
				if(y>slope1*x+(aY-aX*slope1)!=y>slope2*x+(cY-cX*slope2)){ //if above line 1 == above line 2
				//if((y+x)/(((double)h+(double)w))<.5==((double)h/(double)w>y/x)){
					Global.background.setRGB((int)x,(int)y,Color.blue.getRGB());
					}else{
						Global.background.setRGB((int)x,(int)y,Color.red.getRGB());
						}
				//(((h*w)-h)>y/x)
			}
		}
